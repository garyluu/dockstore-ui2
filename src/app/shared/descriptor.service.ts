  ...
  getValidDescriptors(version) {
    if (version) {
      ...
      if (version.validations) {
        ...
        const gValidation = version.validations.find(validation => {
          return validation.type === SourceFile.TypeEnum.DOCKSTOREGXFORMAT2 || validation.type === SourceFile.TypeEnum.DOCKSTOREGXFORMAT2;
        });
        if (gValidation && gValidation.valid) {
          descriptorTypes.push(ToolDescriptor.TypeEnum.GXFORMAT2);
        }
